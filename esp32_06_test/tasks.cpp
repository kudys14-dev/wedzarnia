...
// [NEW v3] taskWeb wywołuje web_server_ws_broadcast() co ~1s
//
// Problem: podczas OTA upload handleClient() blokuje taska na wiele sekund
...
void taskWeb(void* pv) {
    ...
    // [NEW v3] WebSocket broadcast co ~1s
    // pętla co 20ms × 50 = broadcast co 1000ms
    uint32_t wsBroadcastTick = 0;

    for (;;) {
        taskWatchdogs[taskIndex].lastReset = xTaskGetTickCount();

        // web_server_handle_client() obsługuje HTTP + ws.loop() razem
        web_server_handle_client();

        // Broadcast stanu do wszystkich klientów WS co ~1s
        if (++wsBroadcastTick >= 50) {
            wsBroadcastTick = 0;
            web_server_ws_broadcast();
        }
...
void taskMonitor(void* pv) {
    ...
    unsigned long lastNotifyCheck = 0;   // [NEW] ntfy alerts
    for (;;) {
        esp_task_wdt_reset();
        taskWatchdogs[taskIndex].lastReset = xTaskGetTickCount();
        unsigned long now = millis();
        // [NEW] ntfy alerts co 5s
        if (now - lastNotifyCheck > 5000) { lastNotifyCheck = now; notifications_check(); }
...
void tasks_create_all() {
    ...
    xTaskCreatePinnedToCore(taskUI,      "UI",      16384, NULL, 2, NULL, 1);  // [FIX-T1] 16KB: WiFiClientSecure HTTPS GitHub wymaga ~8KB stosu
    ...
    xTaskCreatePinnedToCore(taskWiFi,    "WiFi",    6144,  NULL, 1, NULL, 0);
    xTaskCreatePinnedToCore(taskMonitor, "Monitor", 8192,  NULL, 1, NULL, 0);  // [FIX] 8KB: HTTPClient w notifications_check() wymaga ~4KB stosu

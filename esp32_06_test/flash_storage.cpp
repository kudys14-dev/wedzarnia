// flash_storage.cpp - W25Q128 SPI Flash filesystem
// v2.2 - Naprawa buga wielosektorowego zapisu:
//
// [FIX-13] VERIFY PO KAŻDYM SEKTORZE - poprzednio weryfikowany był tylko
//          pierwszy bajt całego pliku. Przy zapisie 2-sektorowym drugi sektor
//          mógł nie być wymazany (erase cicho fail) i zapis szedł w niewymazany
//          flash → OR z poprzednią zawartością → dane do odczytu były błędne,
//          ale verify pierwszego bajtu przechodziło bo pierwszy sektor był OK.
//          Teraz: po zapisie każdego sektora weryfikowany jest jego pierwszy bajt.
//
// [FIX-14] ERASE VERIFY WSZYSTKICH SEKTORÓW - poprzednio sprawdzane były
//          tylko pierwsze 4 bajty pierwszego sektora. Teraz każdy kasowany
//          sektor jest weryfikowany osobno przed zapisem.
//
// [FIX-15] vTaskDelay W PĘTLI ZAPISU - _flash_write_data_locked trzymało mutex
//          przez ~75ms dla pliku 6KB (16 stron × ~3ms + overhead). TFT task
//          głodził się przez ten czas. Teraz co 8 stron (co ~24ms) oddajemy
//          mutex na 1 tick schedulera.

#include "flash_storage.h"
#include "config.h"
#include <SPI.h>
#include <freertos/task.h>

// ======================================================
// KOMENDY SPI W25Q128
// ======================================================
#define W25Q_CMD_WRITE_ENABLE   0x06
#define W25Q_CMD_WRITE_DISABLE  0x04
#define W25Q_CMD_READ_STATUS1   0x05
#define W25Q_CMD_READ_DATA      0x03
#define W25Q_CMD_PAGE_PROGRAM   0x02
#define W25Q_CMD_SECTOR_ERASE   0x20
#define W25Q_CMD_BLOCK_ERASE_64 0xD8
#define W25Q_CMD_CHIP_ERASE     0xC7
#define W25Q_CMD_JEDEC_ID       0x9F
#define W25Q_CMD_POWER_UP       0xAB

#define W25Q_STATUS_BUSY        0x01
#define SPI_MUTEX_TIMEOUT_MS    2000

static const SPISettings FLASH_SPI_WRITE_SETTINGS(8000000UL, MSBFIRST, SPI_MODE3);
static const SPISettings FLASH_SPI_READ_SETTINGS( 8000000UL, MSBFIRST, SPI_MODE3);

static bool               flashReady    = false;
static FlashFileEntry     fatTable[MAX_FLASH_FILES];
static int                fatEntryCount = 0;
static SemaphoreHandle_t  g_spiMutex    = NULL;

// ======================================================
// MUTEX
// ======================================================
static bool spi_take() {
    if (g_spiMutex == NULL) return true;
    return xSemaphoreTakeRecursive(g_spiMutex, pdMS_TO_TICKS(SPI_MUTEX_TIMEOUT_MS)) == pdTRUE;
}
static void spi_give() {
    if (g_spiMutex != NULL) xSemaphoreGiveRecursive(g_spiMutex);
}

// ======================================================
// CS MANAGEMENT
// ======================================================
static inline void flash_cs_low() {
    digitalWrite(TFT_CS,       HIGH);
    digitalWrite(FLASH_CS_PIN, LOW);
}
static inline void flash_cs_high() {
    digitalWrite(FLASH_CS_PIN, HIGH);
}

// ======================================================
// NISKOPOZIOMOWE (bez mutexa)
// ======================================================
static void _flash_write_enable() {
    flash_wait_busy();
    SPI.beginTransaction(FLASH_SPI_WRITE_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_WRITE_ENABLE);
    flash_cs_high();
    SPI.endTransaction();
    delayMicroseconds(10);
}

uint8_t flash_read_status() {
    SPI.beginTransaction(FLASH_SPI_READ_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_READ_STATUS1);
    uint8_t s = SPI.transfer(0x00);
    flash_cs_high();
    SPI.endTransaction();
    return s;
}

void flash_wait_busy() {
    unsigned long start = millis();
    while (flash_read_status() & W25Q_STATUS_BUSY) {
        if (millis() - start > 10000UL) {
            log_msg(LOG_LEVEL_ERROR, "flash_wait_busy TIMEOUT!");
            break;
        }
        delayMicroseconds(200);
    }
}

static void _flash_read_data(uint32_t address, uint8_t* buffer, uint32_t size) {
    flash_wait_busy();
    SPI.beginTransaction(FLASH_SPI_READ_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_READ_DATA);
    SPI.transfer((address >> 16) & 0xFF);
    SPI.transfer((address >>  8) & 0xFF);
    SPI.transfer( address        & 0xFF);
    for (uint32_t i = 0; i < size; i++) buffer[i] = SPI.transfer(0x00);
    flash_cs_high();
    SPI.endTransaction();
}

static void _flash_write_page(uint32_t address, const uint8_t* data, uint16_t size) {
    if (size > FLASH_PAGE_SIZE) size = FLASH_PAGE_SIZE;
    _flash_write_enable();

    uint8_t statusBefore = flash_read_status();
    if (!(statusBefore & 0x02)) {
        LOG_FMT(LOG_LEVEL_WARN,
            "_flash_write_page: WEL=0 po write_enable! status=0x%02X addr=0x%05lX",
            statusBefore, address);
    }

    SPI.beginTransaction(FLASH_SPI_WRITE_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_PAGE_PROGRAM);
    SPI.transfer((address >> 16) & 0xFF);
    SPI.transfer((address >>  8) & 0xFF);
    SPI.transfer( address        & 0xFF);
    for (uint16_t i = 0; i < size; i++) SPI.transfer(data[i]);
    flash_cs_high();
    SPI.endTransaction();
    flash_wait_busy();
}

static void _flash_erase_sector(uint32_t sectorNumber) {
    uint32_t address = sectorNumber * FLASH_SECTOR_SIZE;
    _flash_write_enable();
    SPI.beginTransaction(FLASH_SPI_WRITE_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_SECTOR_ERASE);
    SPI.transfer((address >> 16) & 0xFF);
    SPI.transfer((address >>  8) & 0xFF);
    SPI.transfer( address        & 0xFF);
    flash_cs_high();
    SPI.endTransaction();
    flash_wait_busy();
}

// ======================================================
// PUBLICZNE NISKOPOZIOMOWE (z mutexem)
// ======================================================
void flash_read_data(uint32_t address, uint8_t* buffer, uint32_t size) {
    if (!spi_take()) { log_msg(LOG_LEVEL_ERROR, "flash_read_data: mutex timeout"); return; }
    _flash_read_data(address, buffer, size);
    spi_give();
}

void flash_write_page(uint32_t address, const uint8_t* data, uint16_t size) {
    if (!spi_take()) { log_msg(LOG_LEVEL_ERROR, "flash_write_page: mutex timeout"); return; }
    _flash_write_page(address, data, size);
    spi_give();
}

void flash_erase_sector(uint32_t sectorNumber) {
    if (!spi_take()) { log_msg(LOG_LEVEL_ERROR, "flash_erase_sector: mutex timeout"); return; }
    vTaskDelay(1);
    _flash_erase_sector(sectorNumber);
    vTaskDelay(1);
    spi_give();
}

void flash_erase_block_64k(uint32_t blockNumber) {
    if (!spi_take()) { log_msg(LOG_LEVEL_ERROR, "flash_erase_block: mutex timeout"); return; }
    uint32_t address = blockNumber * 65536UL;
    _flash_write_enable();
    SPI.beginTransaction(FLASH_SPI_WRITE_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_BLOCK_ERASE_64);
    SPI.transfer((address >> 16) & 0xFF);
    SPI.transfer((address >>  8) & 0xFF);
    SPI.transfer( address        & 0xFF);
    flash_cs_high();
    SPI.endTransaction();
    vTaskDelay(1);
    flash_wait_busy();
    vTaskDelay(1);
    spi_give();
}

void flash_chip_erase() {
    if (!spi_take()) { log_msg(LOG_LEVEL_ERROR, "flash_chip_erase: mutex timeout"); return; }
    _flash_write_enable();
    SPI.beginTransaction(FLASH_SPI_WRITE_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_CHIP_ERASE);
    flash_cs_high();
    SPI.endTransaction();
    spi_give();
    unsigned long start = millis();
    while (millis() - start < 300000UL) {
        vTaskDelay(1);
        delay(500);
        if (!spi_take()) break;
        bool busy = (flash_read_status() & W25Q_STATUS_BUSY);
        spi_give();
        if (!busy) break;
    }
}

uint16_t flash_get_jedec_id() {
    if (!spi_take()) return 0x0000;
    SPI.beginTransaction(FLASH_SPI_READ_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_JEDEC_ID);
    uint8_t mfr  = SPI.transfer(0x00);
    uint8_t type = SPI.transfer(0x00);
    SPI.transfer(0x00);
    flash_cs_high();
    SPI.endTransaction();
    spi_give();
    return ((uint16_t)mfr << 8) | type;
}

// ======================================================
// [FIX-15] ZAPIS WIELOSTRONICOWY - oddaje mutex co 8 stron
// Wywołuje się wewnątrz kodu który JUŻ trzyma mutex.
// Co 8 stron (~24ms) zwalnia mutex na 1 tick żeby TFT mógł
// dokończyć bieżącą operację, po czym pobiera z powrotem.
// ======================================================
static bool _flash_write_data_locked(uint32_t address, const uint8_t* data, uint32_t size) {
    uint32_t offset    = 0;
    uint16_t pageCount = 0;

    while (offset < size) {
        uint16_t pageOff      = (uint16_t)(address % FLASH_PAGE_SIZE);
        uint16_t bytesToWrite = FLASH_PAGE_SIZE - pageOff;
        if (bytesToWrite > (size - offset)) bytesToWrite = (uint16_t)(size - offset);

        _flash_write_page(address, data + offset, bytesToWrite);
        address += bytesToWrite;
        offset  += bytesToWrite;
        pageCount++;

        // [FIX-15] Co 8 stron oddaj mutex na 1 tick schedulera
        // Pozwala TFT taskowi dokończyć operację bez przerywania zapisu
        if (pageCount % 8 == 0 && offset < size) {
            spi_give();
            vTaskDelay(1);
            if (!spi_take()) {
                LOG_FMT(LOG_LEVEL_ERROR,
                    "_flash_write_data_locked: mutex timeout po yield (offset=%lu)", offset);
                return false;
            }
        }
    }
    return true;
}

// ======================================================
// FAT
// ======================================================
static void fat_compact() {
    int writeIdx = 0;
    for (int i = 0; i < MAX_FLASH_FILES; i++) {
        if (fatTable[i].valid == 0xFE) {
            fatTable[i].valid = 0x01;
            log_msg(LOG_LEVEL_WARN, "fat_compact: recovered 0xFE: " + String(fatTable[i].filename));
        }
        if (fatTable[i].valid == 0x01) {
            if (writeIdx != i) memcpy(&fatTable[writeIdx], &fatTable[i], sizeof(FlashFileEntry));
            writeIdx++;
        }
    }
    fatEntryCount = writeIdx;
    for (int i = writeIdx; i < MAX_FLASH_FILES; i++)
        memset(&fatTable[i], 0xFF, sizeof(FlashFileEntry));
}

static void fat_write_to_sector(uint32_t sector) {
    if (!spi_take()) { log_msg(LOG_LEVEL_ERROR, "fat_write: mutex timeout"); return; }
    _flash_erase_sector(sector);

    uint32_t addr = sector * FLASH_SECTOR_SIZE;
    FatHeader hdr;
    hdr.magic      = FAT_MAGIC;
    hdr.entryCount = (uint16_t)fatEntryCount;
    hdr.reserved   = 0;
    uint8_t hdrBuf[sizeof(FatHeader)];
    memcpy(hdrBuf, &hdr, sizeof(FatHeader));
    _flash_write_page(addr, hdrBuf, sizeof(FatHeader));
    addr += sizeof(FatHeader);

    for (int i = 0; i < fatEntryCount; i++) {
        uint8_t entryBuf[sizeof(FlashFileEntry)];
        memcpy(entryBuf, &fatTable[i], sizeof(FlashFileEntry));
        _flash_write_data_locked(addr, entryBuf, sizeof(FlashFileEntry));
        addr += sizeof(FlashFileEntry);
    }
    spi_give();
    LOG_FMT(LOG_LEVEL_DEBUG, "FAT written to sector %lu (%d entries)", sector, fatEntryCount);
}

static void fat_save() {
    fat_compact();
    fat_write_to_sector(FAT_SHADOW_SECTOR);
    fat_write_to_sector(FAT_SECTOR);
    LOG_FMT(LOG_LEVEL_DEBUG, "FAT saved: %d entries", fatEntryCount);
}

static bool fat_load_from_sector(uint32_t sector) {
    uint32_t addr = sector * FLASH_SECTOR_SIZE;
    if (!spi_take()) return false;
    FatHeader hdr;
    _flash_read_data(addr, (uint8_t*)&hdr, sizeof(FatHeader));
    spi_give();

    if (hdr.magic != FAT_MAGIC) {
        LOG_FMT(LOG_LEVEL_WARN, "FAT sector %lu bad magic: 0x%08lX", sector, hdr.magic);
        return false;
    }
    if (hdr.entryCount > MAX_FLASH_FILES) {
        LOG_FMT(LOG_LEVEL_WARN, "FAT sector %lu: entryCount=%u > MAX=%d",
                sector, hdr.entryCount, MAX_FLASH_FILES);
        return false;
    }

    addr += sizeof(FatHeader);
    memset(fatTable, 0xFF, sizeof(fatTable));
    fatEntryCount = 0;

    for (uint16_t i = 0; i < hdr.entryCount; i++) {
        if (!spi_take()) return false;
        _flash_read_data(addr + i * sizeof(FlashFileEntry),
                         (uint8_t*)&fatTable[i], sizeof(FlashFileEntry));
        spi_give();

        if (fatTable[i].valid == 0x01) {
            fatEntryCount++;
        } else if (fatTable[i].valid == 0xFE) {
            log_msg(LOG_LEVEL_WARN,
                "FAT recovery: 0xFE → 0x01: " + String(fatTable[i].filename));
            fatTable[i].valid = 0x01;
            fatEntryCount++;
        }
    }
    LOG_FMT(LOG_LEVEL_INFO, "FAT loaded from sector %lu: header=%u, active=%d",
            sector, hdr.entryCount, fatEntryCount);
    return true;
}

static void fat_load() {
    if (fat_load_from_sector(FAT_SECTOR)) return;
    log_msg(LOG_LEVEL_WARN, "Primary FAT bad, trying shadow...");
    if (fat_load_from_sector(FAT_SHADOW_SECTOR)) {
        log_msg(LOG_LEVEL_INFO, "Shadow FAT OK - restoring primary");
        fat_write_to_sector(FAT_SECTOR);
        return;
    }
    log_msg(LOG_LEVEL_ERROR, "Both FAT corrupted - starting empty");
    fatEntryCount = 0;
    memset(fatTable, 0xFF, sizeof(fatTable));
}

static int fat_find_file(const char* path) {
    for (int i = 0; i < MAX_FLASH_FILES; i++) {
        if (fatTable[i].valid == 0x01 &&
            strncmp(fatTable[i].filename, path, MAX_FILENAME_LEN) == 0)
            return i;
    }
    return -1;
}

static uint16_t fat_find_free_contiguous(uint16_t rangeStart, uint16_t rangeEnd, uint16_t count) {
    for (uint16_t s = rangeStart; s + count - 1 <= rangeEnd; ) {
        bool allFree = true;
        uint16_t blockEnd = s + count - 1;
        for (int i = 0; i < MAX_FLASH_FILES; i++) {
            if (fatTable[i].valid != 0x01) continue;
            uint16_t fs = fatTable[i].startSector;
            uint16_t fe = fs + fatTable[i].sectorCount - 1;
            if (fs <= blockEnd && fe >= s) {
                allFree = false;
                s = fe + 1;
                break;
            }
        }
        if (allFree) return s;
    }
    return 0xFFFF;
}

static void fat_get_sector_range(const char* path, uint16_t& start, uint16_t& end) {
    if      (strncmp(path, "/profiles/", 10) == 0) { start = PROFILES_START; end = PROFILES_END; }
    else if (strncmp(path, "/backup/",    8) == 0)  { start = BACKUPS_START;  end = BACKUPS_END;  }
    else if (strncmp(path, "/logs/",      6) == 0)  { start = LOGS_START;     end = LOGS_END;     }
    else if (strncmp(path, "/web/",       5) == 0)  { start = WEB_START;      end = WEB_END;      }
    else                                             { start = PROFILES_START; end = LOGS_END;     }
}

static int fat_find_free_slot() {
    for (int i = 0; i < MAX_FLASH_FILES; i++)
        if (fatTable[i].valid != 0x01) return i;
    return -1;
}

// ======================================================
// API PUBLICZNE
// ======================================================

bool flash_init(SemaphoreHandle_t spiMutex) {
    if (spiMutex != NULL) {
        g_spiMutex = spiMutex;
        log_msg(LOG_LEVEL_INFO, "Flash: using shared SPI mutex");
    } else {
        if (g_spiMutex == NULL) {
            g_spiMutex = xSemaphoreCreateMutex();
            log_msg(LOG_LEVEL_INFO, "Flash: created own SPI mutex");
        }
    }

    pinMode(FLASH_CS_PIN, OUTPUT);
    flash_cs_high();
    pinMode(TFT_CS, OUTPUT);
    digitalWrite(TFT_CS, HIGH);
    delay(10);

    if (!spi_take()) return false;
    SPI.beginTransaction(FLASH_SPI_READ_SETTINGS);
    flash_cs_low();
    SPI.transfer(W25Q_CMD_POWER_UP);
    flash_cs_high();
    SPI.endTransaction();
    spi_give();
    delay(3);

    {
        if (!spi_take()) return false;
        unsigned long t = millis();
        while ((flash_read_status() & 0x01) && millis() - t < 5000) delay(10);
        uint8_t s = flash_read_status();
        if (s & 0x01) log_msg(LOG_LEVEL_ERROR, "Flash: BUSY timeout po power-up!");
        else LOG_FMT(LOG_LEVEL_INFO, "Flash: idle po power-up, SR1=0x%02X", s);
        spi_give();
    }

    {
        if (!spi_take()) return false;
        uint8_t sr1 = flash_read_status();
        LOG_FMT(LOG_LEVEL_WARN, "Flash SR1: 0x%02X (BP=%d)", sr1, (sr1>>2)&7);
        if ((sr1 & 0x7C) != 0) {
            log_msg(LOG_LEVEL_WARN, "Flash: BP bits ustawione - odblokowanie...");
            SPI.beginTransaction(FLASH_SPI_WRITE_SETTINGS);
            flash_cs_low(); SPI.transfer(0x50); flash_cs_high();
            SPI.endTransaction();
            delayMicroseconds(5);
            SPI.beginTransaction(FLASH_SPI_WRITE_SETTINGS);
            flash_cs_low(); SPI.transfer(0x01); SPI.transfer(0x00); flash_cs_high();
            SPI.endTransaction();
            delay(15);
            uint8_t sr1new = flash_read_status();
            LOG_FMT(LOG_LEVEL_WARN, "Flash SR1 po odblok: 0x%02X", sr1new);
        }
        spi_give();
    }

    uint16_t id  = flash_get_jedec_id();
    uint8_t  mfr = id >> 8;
    LOG_FMT(LOG_LEVEL_INFO, "Flash JEDEC: 0x%04X (mfr=0x%02X)", id, mfr);

    if (mfr == 0x00 || mfr == 0xFF) {
        LOG_FMT(LOG_LEVEL_ERROR, "No flash chip (JEDEC=0x%04X)", id);
        flashReady = false;
        return false;
    }

    fat_load();
    flashReady = true;
    log_msg(LOG_LEVEL_INFO, "SPI Flash ready");
    return true;
}

bool flash_is_ready() { return flashReady; }

bool flash_format() {
    if (!flashReady) return false;
    log_msg(LOG_LEVEL_WARN, "Formatting flash...");
    fatEntryCount = 0;
    memset(fatTable, 0xFF, sizeof(fatTable));

    if (!spi_take()) return false;
    vTaskDelay(1); _flash_erase_sector(FAT_SECTOR);
    vTaskDelay(1); _flash_erase_sector(FAT_SHADOW_SECTOR);
    vTaskDelay(1);
    spi_give();

    fat_write_to_sector(FAT_SECTOR);
    fat_write_to_sector(FAT_SHADOW_SECTOR);
    log_msg(LOG_LEVEL_INFO, "Flash formatted OK");
    return true;
}

bool flash_file_exists(const char* path) {
    return fat_find_file(path) >= 0;
}

// ======================================================
// [FIX-13][FIX-14] flash_file_write - weryfikacja każdego sektora
// ======================================================
bool flash_file_write(const char* path, const uint8_t* data, uint32_t size) {
    if (!flashReady)  { log_msg(LOG_LEVEL_ERROR, "flash_file_write: not ready");  return false; }
    if (size == 0)    { log_msg(LOG_LEVEL_ERROR, "flash_file_write: size=0");     return false; }
    // [FIX-F1] Limit rozmiaru na podstawie rzeczywistego zakresu katalogu,
    // nie stalej 20 sektorow. WEB range = 30 sekt, PROFILES = 100 sekt.
    uint16_t tmpStart, tmpEnd;
    fat_get_sector_range(path, tmpStart, tmpEnd);
    uint32_t maxBytes = (uint32_t)(tmpEnd - tmpStart + 1) * FLASH_SECTOR_SIZE;
    if (size > maxBytes || size > 65536UL) {
        LOG_FMT(LOG_LEVEL_ERROR, "flash_file_write: too large (%lu B, max %lu B)", size, maxBytes);
        return false;
    }

    uint16_t sectorsNeeded = (size + FLASH_SECTOR_SIZE - 1) / FLASH_SECTOR_SIZE;
    uint16_t rangeStart, rangeEnd;
    fat_get_sector_range(path, rangeStart, rangeEnd);

    int oldIdx = fat_find_file(path);
    if (oldIdx >= 0) fatTable[oldIdx].valid = 0xFE;

    uint16_t startSector = fat_find_free_contiguous(rangeStart, rangeEnd, sectorsNeeded);
    if (startSector == 0xFFFF) {
        if (oldIdx >= 0) fatTable[oldIdx].valid = 0x01;
        LOG_FMT(LOG_LEVEL_ERROR, "flash_file_write: no space '%s' (%u sect)", path, sectorsNeeded);
        return false;
    }

    int freeSlot = fat_find_free_slot();
    if (freeSlot < 0) {
        if (oldIdx >= 0) fatTable[oldIdx].valid = 0x01;
        log_msg(LOG_LEVEL_ERROR, "flash_file_write: FAT full");
        return false;
    }

    LOG_FMT(LOG_LEVEL_INFO, "flash_file_write: '%s' %luB sect=%u+%u",
            path, size, startSector, sectorsNeeded);

    if (!spi_take()) {
        log_msg(LOG_LEVEL_ERROR, "flash_file_write: mutex timeout");
        if (oldIdx >= 0) fatTable[oldIdx].valid = 0x01;
        return false;
    }

    // [FIX-14] Erase i weryfikacja KAŻDEGO sektora osobno
    for (uint16_t k = 0; k < sectorsNeeded; k++) {
        uint16_t sect = startSector + k;
        _flash_erase_sector(sect);

        // Weryfikacja erase tego sektora
        uint8_t diagBuf[4] = {0};
        uint32_t diagAddr = (uint32_t)sect * FLASH_SECTOR_SIZE;
        _flash_read_data(diagAddr, diagBuf, 4);

        if (diagBuf[0] != 0xFF || diagBuf[1] != 0xFF) {
            LOG_FMT(LOG_LEVEL_WARN,
                "flash_file_write: erase verify FAIL sect=%u, retrying...", sect);
            _flash_erase_sector(sect);
            _flash_read_data(diagAddr, diagBuf, 4);
            if (diagBuf[0] != 0xFF) {
                LOG_FMT(LOG_LEVEL_ERROR,
                    "flash_file_write: erase FAILED sect=%u after retry!", sect);
                if (oldIdx >= 0) fatTable[oldIdx].valid = 0x01;
                spi_give();
                return false;
            }
        }
        LOG_FMT(LOG_LEVEL_DEBUG, "flash_file_write: sect=%u erased OK", sect);
    }

    // Zapis danych (z yield co 8 stron)
    uint32_t address = (uint32_t)startSector * FLASH_SECTOR_SIZE;
    if (!_flash_write_data_locked(address, data, size)) {
        // mutex już oddany przez _flash_write_data_locked przy błędzie
        if (oldIdx >= 0) fatTable[oldIdx].valid = 0x01;
        log_msg(LOG_LEVEL_ERROR, "flash_file_write: write locked failed");
        // upewnij się że mutex jest oddany
        spi_give();  // bezpieczne dzięki rekurencyjnemu mutexowi
        return false;
    }

    // [FIX-13] Weryfikacja pierwszego bajtu KAŻDEGO sektora
    bool verifyOk = true;
    for (uint16_t k = 0; k < sectorsNeeded; k++) {
        uint32_t sectorAddr = (uint32_t)(startSector + k) * FLASH_SECTOR_SIZE;
        uint8_t  expected;
        uint8_t  actual = 0xFF;

        // Oblicz oczekiwany pierwszy bajt tego sektora z bufora danych
        uint32_t dataOffset = (uint32_t)k * FLASH_SECTOR_SIZE;
        if (dataOffset < size) {
            expected = data[dataOffset];
        } else {
            // Ten sektor jest częściowo lub całkowicie poza danymi - skip
            break;
        }

        _flash_read_data(sectorAddr, &actual, 1);

        if (actual != expected) {
            LOG_FMT(LOG_LEVEL_ERROR,
                "flash_file_write: verify FAILED sect=%u (got 0x%02X, exp 0x%02X)",
                startSector + k, actual, expected);
            verifyOk = false;
            break;
        }
        LOG_FMT(LOG_LEVEL_DEBUG, "flash_file_write: sect=%u verify OK (0x%02X)",
                startSector + k, actual);
    }

    spi_give();

    if (!verifyOk) {
        if (oldIdx >= 0) fatTable[oldIdx].valid = 0x01;
        return false;
    }

    // Aktualizuj FAT
    memset(&fatTable[freeSlot], 0, sizeof(FlashFileEntry));
    strncpy(fatTable[freeSlot].filename, path, MAX_FILENAME_LEN - 1);
    fatTable[freeSlot].filename[MAX_FILENAME_LEN - 1] = '\0';
    fatTable[freeSlot].startSector = startSector;
    fatTable[freeSlot].sectorCount = sectorsNeeded;
    fatTable[freeSlot].fileSize    = size;
    fatTable[freeSlot].valid       = 0x01;

    if (oldIdx >= 0) fatTable[oldIdx].valid = 0x00;
    fat_save();

    LOG_FMT(LOG_LEVEL_INFO, "File written OK: %s (%lu B, sect %u+%u)",
            path, size, startSector, sectorsNeeded);
    return true;
}

bool flash_file_write_string(const char* path, const String& content) {
    if (content.length() == 0) return false;
    return flash_file_write(path, (const uint8_t*)content.c_str(), content.length());
}

int flash_file_read(const char* path, uint8_t* buffer, uint32_t maxSize) {
    if (!flashReady) return -1;
    int idx = fat_find_file(path);
    if (idx < 0) return -1;
    uint32_t readSize = min(fatTable[idx].fileSize, maxSize);
    uint32_t addr     = (uint32_t)fatTable[idx].startSector * FLASH_SECTOR_SIZE;
    if (!spi_take()) return -1;
    _flash_read_data(addr, buffer, readSize);
    spi_give();
    return (int)readSize;
}

String flash_file_read_string(const char* path) {
    if (!flashReady) return "";
    int idx = fat_find_file(path);
    if (idx < 0) return "";
    uint32_t size = fatTable[idx].fileSize;
    if (size == 0) return "";
    if (size > 32768) size = 32768;

    char* buf = (char*)malloc(size + 1);
    if (!buf) { log_msg(LOG_LEVEL_ERROR, "flash_file_read_string: malloc failed"); return ""; }

    uint32_t addr = (uint32_t)fatTable[idx].startSector * FLASH_SECTOR_SIZE;

    // [FIX-16] Czytaj sektorami z oddawaniem mutexa co sektor (~4ms)
    // Zapobiega blokowaniu TFT task podczas odczytu duzych plikow (>4KB)
    uint32_t offset = 0;
    while (offset < size) {
        uint32_t chunkSize = FLASH_SECTOR_SIZE;
        if (chunkSize > size - offset) chunkSize = size - offset;

        if (!spi_take()) {
            free(buf);
            log_msg(LOG_LEVEL_ERROR, "flash_file_read_string: mutex timeout");
            return "";
        }
        _flash_read_data(addr + offset, (uint8_t*)buf + offset, chunkSize);
        spi_give();

        offset += chunkSize;

        // Oddaj czas schedulerowi miedzy sektorami
        if (offset < size) vTaskDelay(1);
    }

    buf[size] = '\0';
    String result(buf);
    free(buf);
    return result;
}

// [FIX-17] Pobierz rozmiar pliku bez odczytu zawartosci
uint32_t flash_file_get_size(const char* path) {
    if (!flashReady) return 0;
    int idx = fat_find_file(path);
    if (idx < 0) return 0;
    return fatTable[idx].fileSize;
}

// [FIX-17] Czytaj fragment pliku (chunk) - dla strumieniowego HTTP
// Zwraca liczbe przeczytanych bajtow, -1 przy bledzie
int flash_file_read_chunk(const char* path, uint32_t offset, uint8_t* buf, uint32_t size) {
    if (!flashReady) return -1;
    int idx = fat_find_file(path);
    if (idx < 0) return -1;
    uint32_t fileSize = fatTable[idx].fileSize;
    if (offset >= fileSize) return 0;
    if (offset + size > fileSize) size = fileSize - offset;
    uint32_t addr = (uint32_t)fatTable[idx].startSector * FLASH_SECTOR_SIZE + offset;
    if (!spi_take()) return -1;
    _flash_read_data(addr, buf, size);
    spi_give();
    return (int)size;
}

bool flash_file_delete(const char* path) {
    int idx = fat_find_file(path);
    if (idx < 0) return false;
    fatTable[idx].valid = 0x00;
    fat_save();
    LOG_FMT(LOG_LEVEL_INFO, "File deleted: %s", path);
    return true;
}

bool flash_file_append(const char* path, const String& content) {
    if (!flashReady || content.length() == 0) return false;
    int idx = fat_find_file(path);
    if (idx >= 0) {
        uint32_t oldSize       = fatTable[idx].fileSize;
        uint32_t newSize       = oldSize + content.length();
        uint32_t allocatedBytes = (uint32_t)fatTable[idx].sectorCount * FLASH_SECTOR_SIZE;

        if (newSize <= allocatedBytes && newSize <= 32768) {
            uint32_t appendAddr  = (uint32_t)fatTable[idx].startSector * FLASH_SECTOR_SIZE + oldSize;
            uint32_t pageOffset  = appendAddr % FLASH_PAGE_SIZE;
            uint32_t spaceInPage = FLASH_PAGE_SIZE - pageOffset;

            if (!spi_take()) return false;
            if (content.length() <= spaceInPage) {
                _flash_write_page(appendAddr, (const uint8_t*)content.c_str(), content.length());
            } else {
                _flash_write_data_locked(appendAddr, (const uint8_t*)content.c_str(), content.length());
            }
            spi_give();

            fatTable[idx].fileSize = newSize;
            fat_save();
            LOG_FMT(LOG_LEVEL_DEBUG, "flash_file_append: fast +%u B → %lu B total",
                    content.length(), newSize);
            return true;
        }

        if (newSize > 32768) {
            String existing = flash_file_read_string(path);
            existing = existing.substring(existing.length() / 2);
            existing += content;
            return flash_file_write_string(path, existing);
        }

        String existing = flash_file_read_string(path);
        existing += content;
        return flash_file_write_string(path, existing);
    }
    return flash_file_write_string(path, content);
}

int flash_list_files(const char* dirPrefix, char files[][MAX_FILENAME_LEN], int maxFiles) {
    int count     = 0;
    int prefixLen = strlen(dirPrefix);
    for (int i = 0; i < MAX_FLASH_FILES && count < maxFiles; i++) {
        if (fatTable[i].valid == 0x01 &&
            strncmp(fatTable[i].filename, dirPrefix, prefixLen) == 0) {
            strncpy(files[count], fatTable[i].filename, MAX_FILENAME_LEN - 1);
            files[count][MAX_FILENAME_LEN - 1] = '\0';
            count++;
        }
    }
    return count;
}

bool flash_mkdir(const char* path) {
    char marker[64];
    snprintf(marker, sizeof(marker), "%s/.dir", path);
    if (flash_file_exists(marker)) return true;
    uint8_t dummy = 0x01;
    return flash_file_write(marker, &dummy, 1);
}

bool flash_dir_exists(const char* path) {
    char dirPrefix[64];
    snprintf(dirPrefix, sizeof(dirPrefix), "%s/", path);
    int prefixLen = strlen(dirPrefix);
    for (int i = 0; i < MAX_FLASH_FILES; i++) {
        if (fatTable[i].valid == 0x01 &&
            strncmp(fatTable[i].filename, dirPrefix, prefixLen) == 0) return true;
    }
    return false;
}

// Zwraca wolne sektory w podanym zakresie
static uint32_t _count_free_in_range(uint16_t rangeStart, uint16_t rangeEnd) {
    uint32_t rangeSize = rangeEnd - rangeStart + 1;
    uint32_t usedInRange = 0;
    for (int i = 0; i < MAX_FLASH_FILES; i++) {
        if (fatTable[i].valid != 0x01) continue;
        uint16_t fs = fatTable[i].startSector;
        uint16_t fe = fs + fatTable[i].sectorCount - 1;
        if (fs > rangeEnd || fe < rangeStart) continue;
        // Czesc lub calosc wpisu lezy w zakresie
        uint16_t overlapStart = (fs > rangeStart) ? fs : rangeStart;
        uint16_t overlapEnd   = (fe < rangeEnd)   ? fe : rangeEnd;
        usedInRange += overlapEnd - overlapStart + 1;
    }
    return (usedInRange < rangeSize) ? (rangeSize - usedInRange) : 0;
}

uint32_t flash_get_free_sectors() {
    // Suma wolnych we wszystkich zakresach
    uint32_t free = 0;
    free += _count_free_in_range(PROFILES_START, PROFILES_END);
    free += _count_free_in_range(BACKUPS_START,  BACKUPS_END);
    free += _count_free_in_range(LOGS_START,     LOGS_END);
    free += _count_free_in_range(WEB_START,      WEB_END);
    return free;
}

uint32_t flash_get_free_sectors_web() {
    return _count_free_in_range(WEB_START, WEB_END);
}

uint32_t flash_get_used_sectors() {
    uint32_t used = 2;  // FAT + shadow
    for (int i = 0; i < MAX_FLASH_FILES; i++)
        if (fatTable[i].valid == 0x01) used += fatTable[i].sectorCount;
    return used;
}

uint32_t flash_get_total_size() { return FLASH_TOTAL_SIZE; }
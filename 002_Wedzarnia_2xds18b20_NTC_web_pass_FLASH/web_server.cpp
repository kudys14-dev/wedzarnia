...
<div class="page-wrap">
<div class="page-header">
<h2>💾 Zarządzanie pamięcią Flash W25Q128</h2>
</div>
...
<div class="row"><span class="lbl">Sektory zajęte</span><span class="val" id="flashUsed">-</span></div>
<div class="row"><span class="lbl">Sektory wolne</span><span class="val" id="flashFree">-</span></div>
</div>
...
<button id="btnFormat" disabled onclick="startFormat()">🗑️ Formatuj pamięć Flash</button>
<div id="progress">
...
.then(r =>r.json())
.then(d =>{
document.getElementById('flashStatus').textContent = d.ok ? '✅ OK':'❌ Brak pamięci';
...
void web_server_init() {
    ...
    server.on("/profile/create", HTTP_POST, []() {
        ...
        // [MOD] Zapis do flash zamiast SD
        if (flash_file_write_string(path.c_str(), data)) {
            server.send(200, "text/plain", "Profil '" + filename + "' zapisany w pamięci flash!");
        } else {
            server.send(500, "text/plain", "Nie można zapisać pliku w pamięci flash.");
        }
    });

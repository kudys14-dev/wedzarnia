...
bool flash_init() {
    ...
    // Ale akceptujemy każdy chip Winbond
    if ((id >> 8) != 0xEF) {
        LOG_FMT(LOG_LEVEL_ERROR, "Unknown flash chip! JEDEC=0x%04X (expected Winbond 0xEF__)", id);
        flashReady = false;
        return false;
    }
    ...
    flashReady = true;
    log_msg(LOG_LEVEL_INFO, "W25Q128 flash initialized OK");
    return true;
}

...
void hardware_init_display() {
    // Inicjalizuj CS piny PRZED wszystkim
    pinMode(TFT_CS,       OUTPUT); digitalWrite(TFT_CS,       HIGH);
    pinMode(FLASH_CS_PIN, OUTPUT); digitalWrite(FLASH_CS_PIN, HIGH);

    // Adafruit_ST7735.initR() wywołuje SPI.begin() wewnętrznie z domyślnym SS pinem (GPIO5).
    // Musimy wywołać SPI.begin() z SS=-1 PO initR() żeby nadpisać tę konfigurację.
    display.initR(INITR_BLACKTAB);

    // Nadpisz konfigurację SPI – SS=-1 eliminuje hardware SS który koliduje
    // z ręcznym zarządzaniem CS przez digitalWrite w flash_storage.cpp
...
// ======================================================
// EKRAN POSTĘPU INICJALIZACJI
// ======================================================
static int bootLineY = 0;

void boot_screen_init() {
    display.fillScreen(ST77XX_BLACK);
    display.setTextSize(1);
    display.setCursor(0, 0);
    display.setTextColor(ST77XX_CYAN);
    display.println(" WEDZARNIA " FW_VERSION);
    display.setTextColor(ST77XX_WHITE);
    display.println("─────────────────────");
    bootLineY = display.getCursorY();
}

void boot_screen_status(const char* label, bool ok) {
    display.setCursor(2, bootLineY);
    display.setTextColor(ST77XX_WHITE);
    display.print(label);
    // wyrównaj status do prawej
    int labelLen = strlen(label);
    int dots = 18 - labelLen;
...
void boot_screen_done() {
    display.setCursor(2, bootLineY + 6);
    display.setTextColor(ST77XX_GREEN);
...
void hardware_init_flash() {
    // SPI już zainicjalizowany w hardware_init_display() – NIE wywołuj SPI.begin/end ponownie.
    // Ponowne SPI.begin() resetuje konfigurację i może powodować glitche na SCK/MOSI.
    digitalWrite(TFT_CS,       HIGH);
    digitalWrite(FLASH_CS_PIN, HIGH);
    delay(10);
...
void display_begin_transaction() {
    SPI_TAKE();
    // Upewnij się że Flash CS=HIGH gdy TFT rysuje – nie dotykamy TFT_CS (Adafruit zarządza nim sama)
    digitalWrite(FLASH_CS_PIN, HIGH);
}

void display_end_transaction() {
    // Upewnij się że Flash CS=HIGH po zakończeniu rysowania TFT
    digitalWrite(FLASH_CS_PIN, HIGH);
    SPI_GIVE();
}

...
// [FIX-1] Usunięto #include <SD.h> – zastąpione flash_storage
// [FIX-2] ui_update_display() i ui_handle_buttons() chronione SPI mutexem
//         przez display_begin_transaction() / display_end_transaction()
// [FIX-3] showDiagnosticsScreen() – SD.cardType() zastąpione statusem flash
// [FIX-4] handleSystemSettingsAction() case 4 – SD.cardType() → flash_is_ready()
// [FIX-5] Etykieta "Karta SD" → "Flash" w menu systemu
...
#include "process.h"
#include "sensors.h"
#include "hardware.h"        // [FIX-1] g_spiMutex, display_begin/end_transaction
#include "flash_storage.h"   // [FIX-1] zamiast <SD.h>
#include <climits>
#include <vector>
#include <ArduinoJson.h>
#include <WiFi.h>
// USUNIĘTO: #include <SD.h>
...
const char* getStateStringForDisplay(ProcessState st) {
    switch (st) {
        ...
        case ProcessState::PAUSE_HEATER_FAULT: return "AWARIA Grzalki";
...
// [FIX-3] Zastąpiono SD.cardType() statusem flash
static void showDiagnosticsScreen() {
    ...
    // [FIX-3] było: SD.cardType() != CARD_NONE
    display.printf("Flash: %s", flash_is_ready() ? "OK" : "ERR");
    display.setCursor(0, 125);
    display.printf("Czas pracy: %lu s", millis() / 1000);
    display.setCursor(0, 140);
    display.printf("Wolne sekt: %lu", flash_get_free_sectors());
}

// ============================================================
// RESET HASLA – długie przytrzymanie ENTER na IDLE
// ============================================================
static void checkAuthResetHold() {
    ...
    if (enterPressed && !holdActive) {
        ...
        if (held >= CFG_AUTH_RESET_HOLD_MS && !resetDone) {
            ...
            LOG_FMT(LOG_LEVEL_INFO, "Auth reset via TFT hold. User: %s", CFG_AUTH_DEFAULT_USER);
...
// ============================================================
// AKCJE MENU USTAWIEN SYSTEMOWYCH
// ============================================================
static void handleSystemSettingsAction() {
    // [FIX-2] Mutex już trzymany przez ui_handle_buttons() przez cały czas
    // tutaj bezpiecznie używamy display.*
    ...
    switch (systemSettingsIndex) {
        ...
        case 2: // Backup
            ...
            // Oddaj mutex na czas operacji flash (flash_storage bierze go wewnętrznie)
            display_end_transaction();
            storage_backup_config();
            display_begin_transaction();
...
// [FIX-CRITICAL] Oddaj SPI mutex na czas oczekiwania na input użytkownika
display_end_transaction();
while (millis() < resetTimeout) {
    ...
    delay(50);
    esp_task_wdt_reset();  // WDT reset w pętli oczekiwania
}
// Odzyskaj SPI mutex przed kontynuacją
display_begin_transaction();
        ...
        case 4: // Informacje systemowe
            {
                ...
                display.setCursor(10, 80);
                // [FIX-4] było: SD.cardType() != CARD_NONE ? "OK" : "ERR"
                display.print("Flash: " + String(flash_is_ready() ? "OK" : "ERR"));
...
// [FIX-CRITICAL] Oddaj SPI mutex na czas oczekiwania na EXIT
display_end_transaction();
...
// [FIX-CRITICAL] Oddaj SPI mutex na czas oczekiwania
display_end_transaction();
...
while (millis() < wifiTimeout) {
    ...
    if (digitalRead(PIN_BTN_EXIT) == LOW) break;
    delay(50);
    esp_task_wdt_reset();
}
// Odzyskaj mutex
...
// [FIX-2] Cała funkcja chroniona przez display_begin/end_transaction()
// ============================================================
void ui_handle_buttons() {
    ...
    // [FIX-2] Bierz mutex SPI przed jakimkolwiek dostępem do display.*
    display_begin_transaction();
...
if (!state_lock()) {
    ...
    continue;  // pomiń obsługę tego przycisku, spróbuj następny
}
...
state_unlock();
                            ...
                            if (pin == PIN_BTN_UP)
                                ...
                                if (sourceMenuIndex == 0) {
                                    ...
                                    if (storage_load_profile()) {
                                        ...
                                        buzzerBeep(3, 200, 100);
                                        log_msg(LOG_LEVEL_ERROR, "Failed to load profile");
                                    }
                                } else {
                                    ...
                                    esp_task_wdt_reset();
                                    // Oddaj mutex na czas pobierania HTTPS
                                    display_end_transaction();
                                    ...
                                    if (ok) {
                                        ...
                                        delay(2000);
                                        log_msg(LOG_LEVEL_ERROR, "Failed GitHub profile: " + selectedProfile);
                                    }
                                }
    ...
    if (!state_lock()) break;  // nie udało się – pomiń
...
if (confirmSelection) {
    ...
    // nawet jeśli lock się nie udał, idziemy do IDLE w UI
                            }
                            currentUiState = UiState::UI_STATE_IDLE;
                            ui_transition_effect(false);
                        }
                    ...
                    case UiState::UI_STATE_CONFIRM_NEXT_STEP:
                        if (pin == PIN_BTN_UP || pin == PIN_BTN_DOWN)
                            ...
                            if (confirmSelection) process_force_next_step();
                            currentUiState = UiState::UI_STATE_IDLE;
                            ui_transition_effect(false);
                        }
                        break;
    ...
    // [FIX-2] Obsługa długiego przytrzymania ENTER (pasek postępu na display)
    checkAuthResetHold();
...
if (state_lock()) {
    ...
    // Nie udało się – użyj ostatniego znanego stanu
    st = lastProcessState;
}
    ...
    // [FIX-2] Bierz mutex SPI przed dostępem do display.*
    display_begin_transaction();
...
if (!state_lock()) {
    // Nie udało się uzyskać mutexa – pomiń tę aktualizację display
    display_end_transaction();
    return;
}
...
state_unlock();
        ...
        switch (currentUiState) {
            ...
            case UiState::UI_STATE_MENU_PROFILES:
                if (profilesLoading) {
                    ...
                    String json_str;
                    if (sourceMenuIndex == 0) {
                        // Oddaj mutex na czas operacji flash
                        display_end_transaction();
                    ...
                    profileList.clear();
                    // [FIX-14] Użyj mniejszego dokumentu + sprawdzaj alokację
                    // Lista profili to tablica krótkich stringów – 2048 B wystarczy
                    {
                        ...
                        if (doc) {
                            ...
                            if (err == DeserializationError::Ok) {
                                ...
                                // Ogranicz liczbę profili do wyświetlanych na ekranie
                                int count = 0;
                                for (JsonVariant value : arr) {
                                    if (count >= 100) break;  // max 100 profili w menu
                                    const char* name = value.as<const char*>();
                                    if (name) profileList.push_back(String(name));
                                    count++;
                                }
                            } else {
                                LOG_FMT(LOG_LEVEL_ERROR, "Profile JSON parse error: %s", err.c_str());
                            }
                            delete doc;
                        } else {
                            log_msg(LOG_LEVEL_ERROR, "Failed to allocate JSON doc for profiles");
                        }
                    }
                ...
                if (profileList.empty()) {
                    ...
                    display.setTextSize(1);
                    for (size_t i = 0; i < profileList.size() && i < 6; i++) {
            ...
            case UiState::UI_STATE_SYSTEM_SETTINGS:
                {
                    ...
                    // [FIX-5] Było "Karta SD" – zmienione na "Flash"
                    const char* settingsItems[] = {
                        "WiFi",
                        "Kalibracja",
                        "Backup",       // backup dotyczy flash, nie SD
                        "Reset statystyk",
                        "Informacje"
                    };
    ...
    display_end_transaction();  // [FIX-2] Oddaj mutex SPI
}

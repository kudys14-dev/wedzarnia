...
// v2.2 - SPI mutex (dzielony z TFT) + wszystkie poprawki krytyczne
#pragma once
#include <Arduino.h>
...
#define LOGS_END            201
#define WEB_START           202
#define WEB_END             241   // 40 sektorow = 120KB na pliki web
...
struct __attribute__((packed)) FlashFileEntry {
    ...
    uint32_t fileSize;
    uint8_t  valid;        // 0x01=OK 0x00=skasowany 0xFF=wolny 0xFE=w trakcie
    uint8_t  reserved[3];
};
...
// [FIX-17] Strumieniowy odczyt - dla HTTP bez alokacji calego pliku
...
uint32_t flash_get_free_sectors_web();  // wolne sektory tylko w /web/ range
uint32_t flash_get_used_sectors();
uint32_t flash_get_total_size();
uint16_t flash_get_jedec_id();

// Niskopoziomowe (dla diagnostyki)

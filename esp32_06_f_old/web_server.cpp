...
// =================================================================
// UPLOAD_HTML - minimalny uploader, fallback gdy /web/upload.html brak
// =================================================================
static const char UPLOAD_HTML[] PROGMEM = R"RAW(<!DOCTYPE html>
<html lang="pl"><head><meta charset="utf-8"><title>Upload Flash</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',sans-serif;background:linear-gradient(135deg,#1a1a2e,#16213e);color:#eee;min-height:100vh;padding:20px}
.box{max-width:520px;margin:0 auto}
.hdr{background:linear-gradient(135deg,#1565c0,#0d47a1);padding:18px;border-radius:14px;margin-bottom:16px}
.hdr h2{margin:0;font-size:1.3em}
.card{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:20px;margin-bottom:14px}
.drop{border:2px dashed rgba(255,255,255,.2);border-radius:10px;padding:30px;text-align:center;cursor:pointer;margin-bottom:12px}
.drop:hover,.drop.over{border-color:#2196f3;background:rgba(33,150,243,.08)}
.drop input{display:none}
.btn{display:block;width:100%;padding:13px;border:none;border-radius:9px;font-size:1em;font-weight:700;cursor:pointer;margin-top:8px}
.btn-go{background:linear-gradient(135deg,#2e7d32,#1b5e20);color:#fff}
.btn-go:disabled{opacity:.4;cursor:not-allowed}
.btn-fmt{background:linear-gradient(135deg,#c62828,#b71c1c);color:#fff}
#log{font-size:.85em;font-family:'Courier New',monospace;margin-top:10px;min-height:20px}
#flog{font-size:.85em;font-family:'Courier New',monospace;margin-top:8px}
.ok{color:#4caf50}.err{color:#f44336}.info{color:#64b5f6}
.flist{list-style:none;margin:0;padding:0}
.flist li{padding:6px 10px;background:rgba(0,0,0,.2);border-radius:6px;margin-bottom:5px;font-size:.88em;display:flex;justify-content:space-between}
</style></head><body>
<div class="box">
<div class="hdr"><h2>📤 Upload plikow na Flash</h2></div>
<div class="card">
<div class="drop" id="dz" ondragover="event.preventDefault();this.classList.add('over')" ondragleave="this.classList.remove('over')" ondrop="event.preventDefault();this.classList.remove('over');addFiles(event.dataTransfer.files)" onclick="g('fi').click()">
<div style="font-size:2em;margin-bottom:8px">📁</div>
<div>Kliknij lub przeciagnij pliki .html .css .js .prof</div>
<input type="file" id="fi" multiple accept=".html,.css,.js,.prof,.txt,.json" onchange="addFiles(this.files)">
</div>
<ul class="flist" id="fl"></ul>
<button class="btn btn-go" id="bu" onclick="uploadAll()" disabled>⬆️ Wgraj wszystkie</button>
<div id="log"></div>
</div>
<div class="card">
<h3 style="color:#ef9a9a;margin-bottom:8px">⚠️ Format Flash</h3>
<p style="font-size:.85em;color:#aaa;margin-bottom:10px">Usuwa wszystkie pliki. Wymagane przy nowym module flash.</p>
<button class="btn btn-fmt" onclick="fmtFlash()">🗑️ Formatuj Flash</button>
<div id="flog"></div>
</div>
<a href="/" style="color:#64b5f6;font-size:.9em">← Strona glowna</a>
</div>
<script>
var files=[];
function g(id){return document.getElementById(id);}
function log(m,c){g('log').innerHTML='<span class="'+(c||'')+'">'+m+'</span>';}
function fmt(b){return b>=1024?(b/1024).toFixed(1)+' KB':b+' B';}
function getPath(n){var e=n.split('.').pop().toLowerCase();return(e==='html'||e==='css'||e==='js'?'/web/':e==='prof'?'/profiles/':'/logs/')+n;}
function addFiles(fl){
  Array.from(fl).forEach(function(f){if(!files.find(function(x){return x.name===f.name;}))files.push(f);});
  var li=g('fl');li.innerHTML='';
  files.forEach(function(f){var el=document.createElement('li');el.innerHTML='<span>'+getPath(f.name)+'</span><span style="color:#666">'+fmt(f.size)+'</span>';li.appendChild(el);});
  g('bu').disabled=files.length===0;
}
async function uploadAll(){
  ...
  for(var i=0;i<files.length;i++){
    var f=files[i];
    var path=getPath(f.name);
    log('Wgrywam: '+f.name+' ('+fmt(f.size)+')...','info');
    try{
      var txt=await f.text();
      var fd=new FormData();
      fd.append('file',new Blob([txt],{type:'text/plain'}),f.name);
      var r=await fetch('/files/upload_raw?path='+encodeURIComponent(path),{method:'POST',body:fd});
      var d=await r.json();
      if(d.ok){ok++;}else{fail++;errs.push(f.name+': '+(d.message||'HTTP '+r.status));}
    }catch(e){fail++;errs.push(f.name+': '+e);}
    await new Promise(function(r){setTimeout(r,400);});
  }
  var msg='OK: '+ok+' | Blad: '+fail;
  if(ok>0)msg+=' - <a href="/" style="color:#64b5f6">Odswiez</a>';
...
function fmtFlash(){
  ...
  g('flog').innerHTML='<span class="info">Formatowanie...</span>';
  fetch('/flash/format',{method:'POST'}).then(function(r){return r.json();}).then(function(d){
    g('flog').innerHTML='<span class="'+(d.ok?'ok':'err')+'">'+(d.ok?'OK: '+d.message:'Blad: '+d.message)+'</span>';
  }).catch(function(e){g('flog').innerHTML='<span class="err">Blad: '+e+'</span>';});
}
...
<div class="drop" id="dropzone" onclick="document.getElementById('fileInput').click()"
  ...
  <div class="hint">index.html, style.css, flash.html, flash.js, calibrate.html…</div>
...
void web_server_init() {
    ...
    struct { const char* url; const char* file; bool noAuth; } pages[] = {
        ...
        { "/calibrate", "/web/calibrate.html", false },
        // /upload - osobny handler z PROGMEM fallback (ponizej)
    ...
    // /upload - z PROGMEM fallback gdy brak pliku na flash
    server.on("/upload", HTTP_GET, [](){
        if (!requireAuth()) return;
        serveWebFile("/web/upload.html", "text/html; charset=utf-8", UPLOAD_HTML);
    });
    ...
    // [FIX] /files/upload_raw - multipart upload dla duzych plikow
    // Uzywa HTTPUpload - jedyna metoda niezawodna dla plikow >8KB
    // Sciezka docelowa w query param: ?path=/web/index.html
    static String _uploadRawPath;
    static String _uploadRawBuf;
    server.on("/files/upload_raw", HTTP_POST,
        [](){
            // Completion handler
            if (!requireAuth()) return;
            if (_uploadRawBuf.length() == 0) {
            ...
            esp_task_wdt_reset();
            if (flash_file_write_string(_uploadRawPath.c_str(), _uploadRawBuf)) {
                server.send(200, "application/json", "{\"ok\":true}");
            } else {
                char msg[128];
                snprintf(msg, sizeof(msg), "Zapis nieudany: %s (%dB) wolne:%lu sekt",
                    _uploadRawPath.c_str(), _uploadRawBuf.length(), flash_get_free_sectors());
        ...
        [](){
            // Upload handler - odbiera chunki
            if (!server.authenticate(storage_get_auth_user(), storage_get_auth_pass())) return;
            HTTPUpload& upload = server.upload();
            if (upload.status == UPLOAD_FILE_START) {
                _uploadRawPath = server.arg("path");
                if (!_uploadRawPath.startsWith("/")) _uploadRawPath = "/" + _uploadRawPath;
                _uploadRawBuf = "";
                esp_task_wdt_reset();
            } else if (upload.status == UPLOAD_FILE_WRITE) {
                esp_task_wdt_reset();
                if (_uploadRawBuf.length() + upload.currentSize <= 32768) {
                    _uploadRawBuf += String((char*)upload.buf).substring(0, upload.currentSize);
                }
            } else if (upload.status == UPLOAD_FILE_END) {
                esp_task_wdt_reset();
            }

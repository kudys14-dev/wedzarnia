...
Serial.printf("[%02lu:%02lu:%02lu] ADC: %.1f\n", h, m, s, adcAvg);
Serial.printf("[%02lu:%02lu:%02lu] R: %.1f\n", h, m, s, resistance);
Serial.printf("[%02lu:%02lu:%02lu] Temp raw: %.2f\n", h, m, s, rawTemp);


if (state_lock()) {
    g_ntcAdc        = adcAvg;
    g_ntcResistance = resistance;
    state_unlock();
}
    ...
    if (!ntcFilterInitialized) {
        ...
        filteredNtcTemp =
            NTC_FILTER_ALPHA * filteredNtcTemp +
            (1.0f - NTC_FILTER_ALPHA) * rawTemp;
    }
